...
const generosDAO = require('../../model/DAO/generos/generos.js')


const inseirNovoGenero = async function (genero, contentType) {
    ...
    try {
        ...
        if(String(contentType).toUpperCase() == 'APPLICATION/JSON'){
            ...
            if (validar){
                ...
                let result = await generosDAO.incertGeneros(genero)
...
const atualizarGeneros = async function(genero, id , contentType) {
    ...
    try {
        ...
        if(String(contentType).toUpperCase() == 'APPLICATION/JSON'){
            ...
            let resultBuscarID = await buscarGeneros(id)
            ...
            if(resultBuscarID.status){
                ...
                if(!validar){
                    ...
                    let result = await generosDAO.updateGeneros(genero)
...
const listarGeneros = async function() {
    ...
    try {
        let result = await generosDAO.selectAllGeneros()
...
const buscarGeneros = async function(id) {
    ...
    try {
        if(id == undefined || id == '' || id == null || isNaN(id)){
            ...
            let result = await generosDAO.selectByIdGeneros(id)
...
const excluirGeneros = async function(id) { 
    let messageJson = JSON.parse(JSON.stringify(config_message))
    ...
    try {
        ...
        if(resultBuscarID.status){
            let result = await generosDAO.deleteGeneros(id)
